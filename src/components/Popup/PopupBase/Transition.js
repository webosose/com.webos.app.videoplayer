 ...
 * @public
 */
const TransitionBase = function ({css, childRef, children, direction, duration, noAnimation,
	timingFunction, type, visible, clipHeight, clipWidth, ...rest}) {
	let innerStyle = {};
	let transitionStyle = '';
	if (type === 'clip' && (direction === 'left' || direction === 'right')) {
		innerStyle =  {width: clipWidth};
	}
	let appendedClassName = classNames(componentCss.transition, rest.className,
		componentCss[direction], componentCss[type]);
	if (visible) {
		appendedClassName = classNames(appendedClassName, componentCss.shown);
	} else {
		appendedClassName = classNames(appendedClassName, componentCss.hidden);
	}
	if (!noAnimation) {
		appendedClassName = classNames(appendedClassName, componentCss[timingFunction],
			componentCss[duration]);
	}
	...
	transitionStyle = getCalculatedStyle();
	rest = {...rest, className: appendedClassName, style: transitionStyle};
	...
	return (
		...
		<div {...rest} >
			<div className={classNames(css.inner, componentCss.inner)} style={innerStyle} ref={childRef}>
				{children}
			</div>
		</div>
	);
